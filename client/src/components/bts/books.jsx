...
function Books() {
  ...
  async function getPaginatedBooks() {
    try {
      ...
      const res = await axios.get(
        `${process.env.REACT_APP_API_BASE_URL}/books?page=${currentPage.current}&limit=${limit}`
      );
  ...
  const handleKeyDown = (event) => {
    if (event.key === "Enter") {
      navigate(`/search?query=${searchTerm}`, { state: { tab: 3 } });
    }
  };
    ...
    <>
      ...
      <section className="course-content mb-[25px]">
        <div className="max-w-[986px] w-full mx-auto px-[4rem] textbooks_title__vCKfs">
          ...
          <div className="add-banner bg-[#edf7ff] flex h-[80px] my-[15px] px-[15px] py-[12px] w-full fos-animate-me fadeInUp delay-0_9">
