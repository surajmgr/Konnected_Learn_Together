...
const pagination = (page, limit, responseData) => {
  ...
  return {
    ...
    next: lastIndex < responseData.length ? { page: page + 1 } : undefined,
    prev: startIndex > 0 ? { page: page - 1 } : undefined,
    result: responseData.slice(startIndex, lastIndex),
  };
};
...
const getQuestion = async (req, res) => {
  try {
    const { qname, qid } = req.params;

    const question = await prisma.question.findFirst({
      where: {
        id: parseInt(qid, 10),
        sqName: { contains: qname, mode: 'insensitive' },
      },
...
const addQuestion = async (req, res) => {
  try {
    const { title, content, topic, uid, date } = req.body;
    const sqName = title.toLowerCase().replace(/[^a-z0-9]/g, '-').slice(0, 25);
    ...
    await prisma.notification.create({
      data: {
        content: `You have asked a question, [${title}].`,
        link: `/question/${sqName}/${newQuestion.id}`,
        userId: uid,
        read: true,
      },
    });

    return res.json("Question created successfully!");
  } catch (error) {
    return res.status(500).json(error.message);
  }
};
...
const updateQuestion = async (req, res) => {
  try {
    ...
    return res.json("Question updated successfully!");
  } catch (error) {
    return res.status(500).json(error.message);
  }
};
...
// Other methods can be similarly refactored.
module.exports = {
  getQuestions,
  getQuestion,
  addQuestion,
