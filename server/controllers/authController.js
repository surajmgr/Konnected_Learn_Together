...
const prisma = require('../utils/prisma'); // assuming you have prisma client set up
const bcrypt = require('bcrypt');
const jwt = require('jsonwebtoken');
const nodemailer = require('nodemailer');
const randomstring = require('randomstring');
...
const securePassword = async (password) => {
    try {
        ...
        console.error("Error in securing password:", error.message);
...
const sendAccountActivationMail = async (name, email, activate_token) => {
    try {
        ...
        console.error("Error sending account activation email:", error.message);
...
const register = async (req, res) => {
    try {
        ...
        if (!emailRegex.test(email)) {
            return res.status(409).json("Invalid email format.");
        }
        ...
        const activateToken = randomstring.generate();
        ...
        await sendAccountActivationMail(name, email, activateToken);
        return res.status(200).json("User has been created and activation email sent!");
    } catch (error) {
...
// Load Activate Account
const loadActivateAccount = async (req, res) => {
    try {
        const activateToken = req.query.activate_token;

        // Check if the user with the given activate_token exists
        const user = await prisma.users.findUnique({ where: { activate_token: activateToken } });

        if (!user) {
            return res.status(404).json("Error when activating. Please check your link.");
        }

        // Update the user's activation status
        await prisma.users.update({
            where: { activate_token: activateToken },
            data: {
                is_activated: true,
                activate_token: '',
            },
        ...
        console.error("Error during account activation:", error.message);
        return res.status(500).json("Internal Server Error.");
    }
};
...
const resetPassword = async (req, res) => {
    try {
        ...
        if (!code) {
            ...
            if (!user) {
                return res.status(404).json("User not found. Please check your email.");
            }

            const resetToken = randomstring.generate(4);
            await prisma.users.update({
                where: { email },
            ...
            await sendResetPasswordMail(user.name, email, resetToken);
            return res.status(200).json("Password reset message has been sent.");
        } else {
            // Verify the reset token and update the password
            const user = await prisma.users.findUnique({
                where: { email, reset_token: code },
            });

            if (!user) {
                return res.status(404).json("Incorrect reset code or email address.");
            }
...
const login = async (req, res) => {
    try {
        ...
        if (!user.is_activated) {
            return res.status(403).json("Please activate your account through the mail.");
        }
        ...
        const { id, name, username, email, avatar, coins } = user;

        // JWT
        
        return res.status(200).json({ id, name, username, email, avatar, token, coins });
    } catch (error) {
